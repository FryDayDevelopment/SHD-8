...
# SmartThingsController maintains a SmartThings object for each location
class SmartThingsController:
    def __init__(self):
        self.locations = []

    # This will create and initialize an ST object for each location where the app is installed
    def initialize(self, refresh=True):
        #This will read by AppID to get the installed location_id for all locations.
        ...
        if r.status_code == 200:
            ...
            for location in data['items']:
                if location['appId'] == ST_WEBHOOK and location['installedAppStatus'] == 'AUTHORIZED':
                    st = SmartThings(location['locationId'])
                    self.locations.append(st)

        for st in self.locations:
            st.initialize(refresh)

    # Return the respective ST object for the requested location
    def getByLocation(self, location_id):
        for st in self.locations:
            if st.location_id == location_id:
                return st

    # Return a list of ST.location dictionaries for the requested locations
    def getLocationData(self, locations):
        location_data = {'locations': []}
        for location in locations:
            st = self.getByLocation(location)
            if st:
                location_data['locations'].append(st.location)
        return location_data
        
    # Return the Guest IP addresses for the requested location
    def getNetworkByLocation(self, location=None):
        ipData = []
        if location:
            query_string = 'select ip from location_network where location_id=?'
            query_val = (location,)
        else:
            query_string = 'select ip from location_network'
            query_val = tuple()
        conn = sqlite3.connect(STDB)
        c1 = conn.cursor()
        ipData = [row[0] for row in c1.execute(query_string, query_val)]
        conn.close()
        return ipData
        
    # Return a list of Guest locations associated with an IP address
    def getLocationsByNetwork(self, ip):
        locations = []
        ipParts = ip.split('.')
        partialIP = '.'.join(ipParts[:3]) + '.%'
        print(f'getLocationsByNetwork({ip}) / partial: {partialIP}')
        conn = sqlite3.connect(STDB)
        c1 = conn.cursor()
        locations = [row[0] for row in c1.execute('select location_id from location_network where ip like ? or ip=?', (partialIP, 'All'))]
        conn.close()
        return locations
...
class SmartThings:
    ...
    def createDB(self):
        ...
        # The local IP addresses for each location (from the perspective of our server)
        create_location_network_table = '''CREATE TABLE IF NOT EXISTS location_network(
            location_network_id INTEGER NOT NULL PRIMARY kEY,
        ...
        cursor.execute(create_location_network_table)

        # Each location where our app is installed
        ...
        # The capabilities of each device and their current state
    ...
    def loadRooms(self):
        ...
        if r.status_code == 200:
            ...
            cursor = conn.cursor()
            roomRows = []
            roomRows = [row[0] for row in cursor.execute('select room_id from room where location_id=?', (self.location_id,))]
...
#                roomRows.append(row[0])
    ...
    def loadDevices(self):
        ...
        if r.status_code == 200:
            ...
            cursor = conn.cursor()
            deviceRows = []
            deviceRows = [row[0] for row in cursor.execute('select device_id from device where location_id=?', (self.location_id,))]
...
#                deviceRows.append(row[0])
    ...
    def readDevices(self):
        ...
        for row in cursor.execute('select * from device where location_id=? and visible=?', (self.location_id,1)):
            ...
            self.devices.append(d_device_id)
    ...
    def loadAllScenes(self):
        ...
        if r.status_code == 200:
            ...
            sceneRows = []
            # Save the current scenes on the table to see if any need to be deleted
            sceneRows = [row[0] for row in c1.execute('select scene_id from scene where location_id=?', (self.location_id,))]
            for scene in data['items']:
                if scene['locationId'] == self.location_id:
                    ...
                    conn.commit()
            # These are the scenes that need to be deleted
                ...
                if pres['deviceId'] == deviceId:
                    for cap in pres['capabilities']:
                        if cap['id'] == capability:
                            ...
                            dev_json = json.dumps({'location_id': self.location_id, 'deviceId': deviceId,'capability': capability, 'value': value})
    ...
    def changeDevice(self, deviceId, capability, value, user=None):
        ...
        #Make sure the device is at this location first.
        if not deviceId in self.devices:
            print(f'Device not at this location: {deviceId}')
            return False
        ...
        # Just an FYI on the remaining rate limits
        print(f'X-RateLimit-Limit: {r.headers["X-RateLimit-Limit"]}')
        print(f'X-RateLimit-Remaining: {r.headers["X-RateLimit-Remaining"]}')
        print(f'X-RateLimit-Reset: {r.headers["X-RateLimit-Reset"]}')
        print (r.text)
        if r.status_code == 200:
            return True
    ...
    def changeThermostat(self, settings, user=None):
        ...
        #Make sure the device is at this location first.
        if not settings['deviceId'] in self.devices:
            print(f'Thermostat not at this location: {settings["deviceId"]}')
            return False
    ...
    def runScene(self, scene_id, user=None):
        ...
        #Make sure the scene is at this location first.
        if not scene_id in self.scenes:
            print(f'Scene not at this location: {scene_id}')
            return False
    ...
    def getConfig(self):
        ...
        for loc in c1.execute('select location_id, name, nickname, email from location where location_id=?', (self.location_id,)):
            newLocation = {'location_id': loc[0], 'name': loc[1], 'nickname': loc[2], 'email': loc[3], 'network': []}
        networkData = []
        networkData = [{'location_network_id': network[0], 'ip': network[1]} for network in c1.execute('select location_network_id, ip from location_network where location_id=?', (self.location_id,))]
#        for network in c1.execute('select location_network_id, ip from location_network where location_id=?', (self.location_id,)):
#            networkData.append({'location_network_id': network[0], 'ip': network[1]})
        newLocation['network'] = networkData
        ...
        if len(configData['location']) > 0:
            for item in configData['location']:
                if item.get('location_id', ''):
                    ...
                    networkData = item['network']
            print('nickname: %s / email: %s / network: %s' % (nickname, email, networkData))
            c1.execute('update location set nickname=?, email=? where location_id=?', (nickname, email, location_id))
            status = True
            c1.execute('delete from location_network where location_id=?', (location_id,))
            for network in networkData:
                c1.execute('insert into location_network (location_id, ip) values (?,?)', (location_id, network['ip']))
    ...
    def getPresence(self):
        ...
        config = {'location_id': self.location_id, 'presence': []}

        conn = sqlite3.connect(STDB)
        conn.row_factory = sqlite3.Row
        c1 = conn.cursor()

        config['presence'] = [dict(sensor) for sensor in c1.execute('select device_id, label, seq, visible, nickname from device where location_id=? and category=?', (self.location_id, 'MobilePresence'))]
...
#            config['presence'].append(dict(sensor))
    ...
    def getScenes(self):
        ...
        config = {'location_id': self.location_id, 'scenes': []}
...
if __name__ == '__main__': #This will only be True if we are directly running this file for testing.
    ...
    stc.initialize(refresh=False)
    for st in stc.locations:
        print(f'location_id: {st.location_id}')
